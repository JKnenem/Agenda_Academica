		...
		<div id="list-materia" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="nome" title="${message(code: 'materia.nome.label', default: 'Nome')}" />
					
						<g:sortableColumn property="professor" title="${message(code: 'materia.professor.label', default: 'Professor')}" />
				...
				<g:each in="${materiaInstanceList}" status="i" var="materiaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${materiaInstance.id}">${fieldValue(bean: materiaInstance, field: "nome")}</g:link></td>
					
						<td>${fieldValue(bean: materiaInstance, field: "professor")}</td>
